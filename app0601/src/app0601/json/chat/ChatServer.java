...
public class ChatServer extends JFrame{
	...
	ServerSocket server;
	Thread serverThread; //서버 가동에 사용되는 쓰레드, 특히 접속자 감지에서 메인쓰레드로 하여금 대기상태에 빠지지 않게하기 위해 필요함
	
	Vector<ServerMsgThread> clientList = new Vector<ServerMsgThread>(); //명단 저장할 것임
	...
	public void runServer() {
		...
		try {
			...
			while(serverFlag) {
				...
				//메시지 담당 쓰레드 객체 생성 후 벡터 명단에 추가!!1
				ServerMsgThread smt = new ServerMsgThread(socket, this);
				smt.start(); //열심히 듣고 말하기를 시작!!
