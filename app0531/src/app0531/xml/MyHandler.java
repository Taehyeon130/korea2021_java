...
//파싱 대상이 되는 XML문서의 모든 노드를 만날때마다 개발자가 이벤트를 통해 정보를 전달해주는 핸들러 객체 정의
//개발자는 핸들러가 발생시키는 이벤트에 따라 원하는 파싱 관련 작업을 진행 할 수 있다.
public class MyHandler extends DefaultHandler{
	//파싱을 진행하는 실행부가 지금 어느 태그를 지나치는지를 알 수있는 표시 용도의 논리값을 선언
	boolean isMembers;
	boolean isMember;
	boolean isName;
	boolean isAge;
	...
	public void characters(char[] ch, int start, int length) throws SAXException {
		...
		if(isName) { //현재 실행부가 name시작 태그를 막 지나친 상태라면
			member.setName(data);
		}else if(isAge) {
	...
	public void endDocument() throws SAXException {
		System.out.println("문서 끝!! , 최종적으로 분석된 회원 수는 "+memberList.size());
